...
class TestMultiSimulationManager:
    ...
    RESULT_DIR = Path(__file__).parent / "test_results" / "test_multi_simulation_manager"
    RESULT_DIR.mkdir(exist_ok=True)

    def _evaluate(self, scenario_list, result_dir, n_workers=2):

        multi_simulation_manager = MultiSimulationManager(scenario_list, result_dir=result_dir, n_workers=n_workers)
        test_result = multi_simulation_manager.simulate()

        n_expected_results = 7
        n_actual_results = len(test_result)
        assert n_actual_results == n_expected_results, f"Expected {n_expected_results} test results, got {n_actual_results}"
