...
class AdvancedMpcController:
    ...
    def __init__(self,
        ...
        self._reference_values = {
            var_name: np.zeros((self.n_horizon,)) for var_name in ("p_x", "p_y", "v", "theta", "initial_dhw", "lead_v_pred")
        }
        self._reference_values_set = False  # whether or not the reference trajectory has been set yet
        # Buffer
    ...
    def _create_model(self) -> do_mpc.model.Model:
        ...
        delta = model.set_variable(var_type="_x", var_name="delta", shape=(1, 1))  # wheel steer angle
        v = model.set_variable(var_type="_x", var_name="v", shape=(1, 1))  # velocity
        theta = model.set_variable(var_type="_x", var_name="theta", shape=(1, 1))  # heading
        model.set_variable(var_type="_x", var_name="dhw", shape=(1, 1))  # dhw (distance to lead vehicle; -1 if no lead vehicle is present)

        # Define the control inputs of the model
        delta_v = model.set_variable(var_type="_u", var_name="delta_v", shape=(1, 1))  # wheel steer rate
        a = model.set_variable(var_type="_u", var_name="a", shape=(1, 1))  # longitudinal acceleration (in theta direction)
        ...
        model.set_variable(var_type="_tvp", var_name="theta_set")  # orientation setpoint
        model.set_variable(var_type="_tvp", var_name="v_set")  # velocity setpoint
        initial_dhw = model.set_variable(var_type="_tvp", var_name="initial_dhw")  # dhw at this timestep (constant over the whole horizon)
        lead_v_pred = model.set_variable(var_type="_tvp", var_name="lead_v_pred")  # absolute velocity of lead vehicle predicted over the horizon (-1 if there is no lead vehicle)

        # Define a parameter of the model (the wheel base of the vehicle)
        self._l_wb = self._vehicle_parameters.a + self._vehicle_parameters.b  # wheel base (distance between front and rear axle)

        # Set the right-hand side (RHS) of the differential equation for each state variable
        model.set_rhs("p_x", v * np.cos(theta))  # x-position is updated based on velocity and orientation
        model.set_rhs("p_y", v * np.sin(theta))  # y-position is updated based on velocity and orientation
        model.set_rhs("delta", delta_v)  # wheel steer angle is updated based on wheel steer rate
        model.set_rhs("v", a)  # velocity is updated based on acceleration
        model.set_rhs("theta", (v / self._l_wb) * np.tan(delta))  # orientation is updated based on velocity, steering angle, and vehicle length
    ...
    def _create_mpc(self) -> do_mpc.controller.MPC:
        ...
        lterm_theta = (np.arctan2(np.sin(self._model.x["theta"] - self._model.tvp["theta_set"]), np.cos(self._model.x["theta"] - self._model.tvp["theta_set"]))) ** 2

        # -- Longitudinal error --
        # If no lead is present or it is far away, default value is "-1"
        no_lead_present = self._model.tvp["initial_dhw"] == -1
        # a) Velocity error
        lterm_v = if_else(no_lead_present, (self._model.x["v"] - self._model.tvp["v_set"]) ** 2, 0.0)
        # b) THW error
        ...
        set_thw_with_lead_pow = (self._min_dhw_safety / self._model.x["v"]) + self._min_thw
        # Actual thw at the moment
        current_thw_error = (set_thw_with_lead_pow - (self._model.x["dhw"] / self._model.x["v"])) ** 2
        lterm_thw = self._cost_function_weights["thw"] * if_else(no_lead_present, thw_error_no_lead, current_thw_error)

        lterm = lterm_cte + lterm_theta + lterm_v + lterm_thw

        mterm = self.m_term_scale * lterm
        mpc.set_objective(lterm=lterm, mterm=mterm)  # set the objective function for the MPC controller
        ...
        mpc.bounds["lower", "_x", "delta"] = - self._delta_lim  # lower bound on steering angle
        mpc.bounds["upper", "_x", "delta"] = self._delta_lim # upper bound on steering angle
        mpc.bounds["lower", "_x", "v"] = self._v_min  # lower bound on velocity
        mpc.bounds["upper", "_x", "v"] = self._v_max  # upper bound on velocity

        mpc.bounds["lower", "_u", "delta_v"] = - self._delta_v_lim  # lower bound on steering rate
        mpc.bounds["upper", "_u", "delta_v"] = self._delta_v_lim  # upper bound on steering rate
        ...
        def tvp_fun(t_now: float) -> castools.structure3.SXStruct | castools.structure3.MXStruct:  # noqa: ARG001

            for k in range(self.n_horizon):
                tvp_template["_tvp", k, "p_x_set"] = self._reference_values["p_x"][k]  # x-position setpoint
                tvp_template["_tvp", k, "p_y_set"] = self._reference_values["p_y"][k]  # y-position setpoint
                tvp_template["_tvp", k, "v_set"] = self._reference_values["v"][k]  # velocity setpoint
                tvp_template["_tvp", k, "theta_set"] = self._reference_values["theta"][k]  # orientation setpoint
                tvp_template["_tvp", k, "initial_dhw"] = self._reference_values["initial_dhw"][k]
                tvp_template["_tvp", k, "lead_v_pred"] = self._reference_values["lead_v_pred"][k]
    ...
    def _log_prediction_info(self) -> None:
        ...
        predicted_state_values = {
            n: np.array(self._mpc.data.prediction(("_x", n))).reshape(self.n_horizon + 1) for n in self._mpc.x0.keys()  # noqa: SIM118
        }
        ...
        dtheta = np.arctan2(np.sin(predicted_theta - ref_theta), np.cos(predicted_theta - ref_theta))
        dtheta_pow = dtheta ** 2
        ...
        # Thw error
        cur_set_thw = (self._min_dhw_safety / predicted_state_values["v"][1:]) + self._min_thw
        if no_lead_present:
            # If no lead is present or it is far away, thw does not matter in the cost function
            cur_thw_error = [0.0]
            lterm_thw = 0.0
        else:
            cur_thw = (predicted_state_values["dhw"][1:] / predicted_state_values["v"][1:])
    ...
    def _update_reference_trajectory(self,
    ...
    def make_step(self,
        ...
        self._update_reference_trajectory(ref_trajectory, ref_trajectory_theta, ref_velocity, cur_dhw, cur_lead_v)
        ...
        if not self._initial_guess_set:
            self._mpc.x0 = np.array(current_state).reshape(-1, 1)  # set the initial state of the system
            self._mpc.set_initial_guess()  # set the initial guess for the optimization problem
            self._initial_guess_set = True  # set the flag to indicate that the initial guess has been set
