...
class SingleSimulationEvaluator:
    def __init__(
        ...
        self._expected_termination_reason = scenario_description[
        ...
        self._a_max_long = (
            4 * 1.01 if self._operation_mode_index == "1.2" else 6 * 1.01
        )  # m/s^2
        self._a_max_lat = (
            1 * 1.01 if self._operation_mode_index == "1.2" else 2 * 1.01
        )  # m/s^2
        self._thw_min = (
            1.8 * 1.01 if self._operation_mode_index == "1.2" else 0.9 * 1.01
        )  # s
    ...
    def generate_plot(
        ...
        *mark,
        ...
        if y2:
            ...
            f.savefig(
                self._result_dir / f"{self._scenario_name}_{filename}",
                format="png",
                dpi=300,
            )
            plt.close(f)
    ...
    def calc_metrics(self) -> None:
        ...
        for time_step, ego_state in enumerate(self._ego_states):
            ...
            # Find lead vehicle
            ego_llt_id = self._lanelet_wrapper.find_lanelet_id_by_position(
                ego_state["x"], ego_state["y"]
            )
            surrounding_vehicles = self._lanelet_wrapper.find_surrounding_vehicles(
                ego_state["x"], ego_state["y"], other_obj_obs
            )
            lead_vehicle = surrounding_vehicles.lead

            # Append ego_s to list
            ego_s, ego_t = self._lanelet_wrapper.from_cart_to_llt_frenet(
                ego_llt_id, ego_state["x"], ego_state["y"]
            )
            ...
            if lead_vehicle:
                ...
                lead_s, lead_t = self._lanelet_wrapper.from_cart_to_llt_frenet(
                ...
                self._collision_speed = (
                    self._v_long_ego_list[-1] - self._v_long_lead_list[-1]
                )
    ...
    def plot_scenario(self) -> None:
        ...
        self.generate_plot(
            ...
            "ttc [s]",
            "ttb [s]",
            filename="t-ttc-ttb.png",
            lower_limit=0,
            upper_limit=self._scenario.duration,
        )
        ...
        self.generate_plot(
            ...
            "t-ttb.png",
            [0, 1, "red"],
            [self._scenario_limits["limits"]["min_ttb_best_case"] - 0.05, 0.1, "green"],
        )
        ...
        self.generate_plot(
            self._t,
            [v * 3.6 for v in self._v_long_ego_list],
            self._a_long_list,
            "t [s]",
            "v [m/s]",
            "a long [m/s^2]",
            filename="t-v-a_long.png",
        )
        ...
        self.generate_plot(
            self._t,
            [i * 3.6 for i in self._v_long_ego_list],
            None,
            "t [s]",
            "v [m/s]",
            None,
            0,
            self._scenario_speed_limit + 20,
            "v.png",
    ...
    def check_success_criteria(self) -> dict:
        ...
        success_criteria = {
            ...
            "a_long_passed": False,
            "a_lat_passed": False,
            "thw_min_passed": False,
            "v_max_passed": False,
            "goal_reached": False,
            "reacted": False,
            "low_collision_speed": False,
        }
        ...
        if (
            max(abs(i) for i in self._a_long_list) > 4
        ...
        if self._operation_mode_index in ("1.1", "1.2"):
            if False in [
                success_criteria["a_lat_passed"],
                success_criteria["a_long_passed"],
                success_criteria["collision_passed"],
                success_criteria["thw_min_passed"],
                success_criteria["v_max_passed"],
                success_criteria["goal_reached"],
            ]:
                success_criteria["passed"] = False
        else:  # noqa: PLR5501
            if False in [
                success_criteria["reacted"],
                success_criteria["low_collision_speed"],
            ]:
                success_criteria["passed"] = False
    ...
    def create_test_report(self) -> None:  # noqa: PLR0912
        ...
        if self._operation_mode_index == "1.2":
            data = [
                ...
                [
                    "1: no Collision",
                    ("passed" if success_criteria["collision_passed"] else "failed"),
                ],
                [
                    "2: Longitudinal acceleration",
                    ("passed" if success_criteria["a_long_passed"] else "failed"),
                ],
                [
                    "3: Lateral acceleration",
                    ("passed" if success_criteria["a_lat_passed"] else "failed"),
                ],
                [
                    "4: Minimal distance",
                    ("passed" if success_criteria["thw_min_passed"] else "failed"),
                ],
                [
                    "5: Maximum speed",
                ...
                [
                    f"6: Expected goal '{self._expected_termination_reason}' reached",
                    ("passed" if success_criteria["goal_reached"] else "failed"),
                ],
            ]
        elif self._operation_mode_index == "1.1":
            data = [
            ...
            data = [
                ...
                [
                    "1: System reacted",
                    ("passed" if success_criteria["reacted"] else "failed"),
                ],
                [
                    "2: Speed at collision <= 45km/h",
                    (
                        f"passed -> v = {round(self._collision_speed * 3.6)} km/h"
                        if success_criteria["low_collision_speed"]
                        else f"failed -> v_col = {round(self._collision_speed * 3.6)} km/h"
                    ),
                ],
            ]
        ...
        ## create PDF
        c = canvas.Canvas(
            str(self._report_dir / f"{self._scenario.id}_report.pdf"), pagesize=A4
        )
        w, h = A4
        ...
        # Plot scenario
        c.drawImage(
            self._result_dir / f"{self._scenario_name}.png",
            30,
            h - 120,
            width=35 * 15,
            height=35,
        ...
        c.drawImage(
            self._result_dir / f"{self._scenario_name}_a_long.png",
            20,
            h - 550,
            width=180 * 1.6,
            height=180,
        ...
        c.drawImage(
            self._result_dir / f"{self._scenario_name}_a_lat.png",
            300,
            h - 550,
            width=180 * 1.6,
            height=180,
        ...
        c.drawImage(
            self._result_dir / f"{self._scenario_name}_thw.png",
            20,
            h - 750,
            width=180 * 1.6,
            height=180,
        ...
        c.drawImage(
            self._result_dir / f"{self._scenario_name}_v.png",
            300,
            h - 750,
            width=180 * 1.6,
            height=180,
        ...
        c.drawString(
            97,
            h - 80,
            "area represents an area that is considered critical by a human driver.",
        )
        # Second row
        ...
        c.drawString(
            105,
            h - 95,
            " value is the best-case reaction if the system were to decelerate to the maximum",
        )
        # Third row
        c.drawString(
            50,
            h - 110,
            "immediately at the start of the scenario. It describes the minimum TTS/TTB occurring",
        )
        c.drawString(
            50,
            h - 125,
            "during immediate deceleration with maximum longitudinal deceleration.",
        )

        ## Plots
        c.drawImage(
            self._result_dir / f"{self._scenario_name}_t-ttb.png",
            50,
            h - 450,
            width=280 * 1.6,
            height=280,
        ...
        c.drawImage(
            self._result_dir / f"{self._scenario_name}_t-tts.png",
            50,
            h - 730,
            width=280 * 1.6,
            height=280,
        ...
        output = {
            ...
            "operation_mode": self._operation_mode_index,
            "success_criteria": success_criteria,
        }
        with (self._report_dir / f"{self._scenario.id}_report.json").open(
            "w", newline=""
        ) as file:
